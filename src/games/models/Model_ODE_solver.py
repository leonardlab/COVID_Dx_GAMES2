...
class Mechanism_Solver(object):
    ...
    def set_initial_condition(self, x_init: np.ndarray) -> None:
        ...
        ---------- 
        x_init
            a numpy array of the initial conditions for each 
            model state
    ...
    def set_rates(self, rates: np.ndarray) -> None:
        ...
        ----------
        rates:
            a numpy array of the rates. Contains
    ...
    def solve(self, tspace: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:   
        ...
        ----------
        tspace 
            a numpy array defining the time points for which to solve
            the model ODEs

        Returns
        -------
        self.solution
            a numpy array definng the simulation values at all 
...
class ODE_solver(Mechanism_Solver):
    """Derived class of Mechanism_Solver. Used for
        models A-C
    ...
    def jacobian(
        ...
        ----------
        t
            a numpy array defining the time points (necessary
            parameter for the ODE solver)

        x
            a numpy array defining the initial conditions (necessary
            parameter for the ODE solver)

        rates
            a numpy array of the parameter values for the ODE solver
    ...
    def gradient(
        ...
        -------
        velocity
            a numpy array defining the gradient for the 
            ODE solver
...
###############################################
# ODE Solver for mechanism D with separated free parameters that were set equal
###############################################  
class ODE_solver_D_separate_free_plus_fixed_params(Mechanism_Solver):
    """Derived class of Mechanism_Solver, used for 
        model D with separated parameters that were set equal to each
        other in model D, and fixed parameters set in the config.json
        file (used for sensitivity analysis of all model parameters)
    ...
    def make_jacobian(self):
        ...
        k_degv, k_bvu, k_bvp1, k_bup2, k_RTon, k_RToff, k_RNaseon, k_RNaseoff, k_bcp1, k_bcp2, k_T7on, k_T7off, k_FSS, a_RHA, b_RHA, c_RHA, k_SSS, k_txn_p, k_cas13, k_degRrep =\
        sp.symbols('k_degv, k_bvu, k_bvp1, k_bup2, k_RTon, k_RToff, k_RNaseon, k_RNaseoff, k_bcp1, k_bcp2, k_T7on, k_T7off, k_FSS, a_RHA, b_RHA, c_RHA, k_SSS, k_txn_p, k_cas13, k_degRrep')
     
        rates = [k_degv, k_bvu, k_bvp1, k_bup2, k_RTon, k_RToff, k_RNaseon, k_RNaseoff, k_bcp1, k_bcp2, k_T7on, k_T7off, k_FSS, a_RHA, b_RHA, c_RHA, k_SSS, k_txn_p, k_cas13, k_degRrep]
        ...
        #Rates
        C_scale =  10 ** 6
        u_v = - k_degv*x_v*x_aCas13 - k_bvu*x_v*x_u - k_bvp1*x_v*x_p1
        u_p1v =  k_bvp1*x_v*x_p1/C_scale - k_degv*x_p1v*x_aCas13 - k_RTon*x_p1v*x_RT + k_RToff*x_RTp1v
        u_p2u = k_bup2*x_u*x_p2 - k_degv*x_p2u*x_aCas13 - k_RTon*x_p2u*x_RT + k_RToff*x_RTp2u
        u_p1cv = k_degv*x_p1v*x_aCas13 - k_RTon*x_p1cv*x_RT + k_RToff*x_RTp1cv
        u_p2cu = k_degv*x_p2u*x_aCas13 - k_RTon*x_p2cu*x_RT + k_RToff*x_RTp2cu
        u_RTp1v = - k_RToff*x_RTp1v + k_RTon*x_RT*x_p1v - k_degv*x_RTp1v*x_aCas13 - k_FSS*x_RTp1v
        u_RTp2u = - k_RToff*x_RTp2u + k_RTon*x_RT*x_p2u - k_degv*x_RTp2u*x_aCas13 - k_FSS*x_RTp2u
        ...
        u_RNasecDNA1v = - k_RHA*x_RNasecDNA1v - k_RNaseoff*x_RNasecDNA1v + k_RNaseon*x_RNase*x_cDNA1v
        u_RNasecDNA2u = - k_RHA*x_RNasecDNA2u - k_RNaseoff*x_RNasecDNA2u + k_RNaseon*x_RNase*x_cDNA2u
        u_cDNA1 = k_RHA*x_RNasecDNA1v - k_bcp2*x_cDNA1*x_p2
        u_cDNA2 = k_RHA*x_RNasecDNA2u - k_bcp1*x_cDNA2*x_p1
        u_p2cDNA1 =  k_bcp2*x_cDNA1*x_p2 + k_RToff*x_RTp2cDNA1 - k_RTon*x_RT*x_p2cDNA1
        u_p1cDNA2 = k_bcp1*x_cDNA2*x_p1 + k_RToff*x_RTp1cDNA2 - k_RTon*x_RT*x_p1cDNA2
        u_RTp2cDNA1 = k_RTon*x_RT*x_p2cDNA1 - k_RToff*x_RTp2cDNA1 - k_SSS*x_RTp2cDNA1
        u_RTp1cDNA2 = k_RTon*x_RT*x_p1cDNA2 - k_RToff*x_RTp1cDNA2 - k_SSS*x_RTp1cDNA2
        u_pro = k_SSS*x_RTp2cDNA1 + k_SSS*x_RTp1cDNA2 - k_T7on*x_T7*x_pro + k_T7off*x_T7pro + k_txn*x_T7pro
        u_T7pro = - k_T7off*x_T7pro + k_T7on*x_T7*x_pro - k_txn*x_T7pro
        u_u = k_txn*x_T7pro - k_bvu*x_u*x_v/C_scale - k_degv*x_u*x_aCas13 - k_cas13*x_u*x_iCas13 - k_bup2*x_u*x_p2
        u_Cas13 = k_cas13*x_u*x_iCas13
        u_uv = k_bvu*x_u*x_v/C_scale
        u_qRf = - k_degRrep*x_aCas13*x_qRf
    ...
    def gradient(
        ...
        k_degv, k_bvu, k_bvp1, k_bup2, k_RTon, k_RToff, k_RNaseon, k_RNaseoff, k_bcp1, k_bcp2, k_T7on, k_T7off, k_FSS, a_RHA, b_RHA, c_RHA, k_SSS, k_txn, k_cas13, k_degRrep = rates
